...
server.post('/', async (req, res) => {
  ...
  if (!name || !QuestionId || !AnswerId || !QuizAttemptId)
    return res.status(400).send('No se han completado los datos necesarios');
...
server.put('/:id', async (req, res) => {
  ...
  if (!instance)
    return res.status(404).send('No se ha encontrado la instancia solicitada');
  try {
